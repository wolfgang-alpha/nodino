# TODO

## Done

- Speech-to-text via faster-whisper (medium.en model, int8 quantization)
- Mempalace integration (semantic search + knowledge graph)
- Claude Code CLI as AI backbone (Max subscription, MCP tools)
- Mempalace MCP server for tool-use access
- CalDAV integration via MCP

## Future

- Email integration — connect to inbox, auto-extract todos and appointments, add them to kanban and calendar
- Claude Code mempalace bridge — let local Claude Code sessions read/write to mempalace, automatically storing coding session insights
- Multi-language STT support (currently English only)
- Conversation mining — ingest past conversations into mempalace
- Project file mining — import documents/notes into the memory palace
- Mobile-optimized UI (responsive layout for small screens)
